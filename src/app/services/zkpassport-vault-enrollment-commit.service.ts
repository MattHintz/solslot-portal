...
export class ZkPassportVaultEnrollmentCommitService {
  private readonly coinset = inject(CoinsetService);
  private readonly session = inject(SessionService);

  async commitAuthorizedEnrollment(
    authorization: ZkPassportVaultEnrollmentAuthorizationResult,
    options: EnrollmentCommitOptions = {},
  ): Promise<ZkPassportVaultEnrollmentCommitResult> {
    ...
    while (true) {
      if (lastVault?.current_coin_id) {
        ...
        if (currentCoinId !== beforeCoinId) {
          throw new Error(
            `vault enrollment commit: confirmed vault advanced to unexpected coin ${currentCoinId}`,
          );
        }
      }
      if (Date.now() >= deadline) {
        throw new Error('vault enrollment commit: timed out waiting for confirmed vault enrollment');
      }
...
export interface EnrollmentCommitOptions {
  timeoutMs?: number;
  pollIntervalMs?: number;
  delayMsOverride?: number;
}

export interface ZkPassportVaultEnrollmentCommitResult extends ZkPassportVaultEnrollmentAuthorizationResult {
