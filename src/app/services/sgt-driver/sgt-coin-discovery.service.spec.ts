...
import {
  DiscoveryResult,
  SgtCoinDiscoveryService,
} from './sgt-coin-discovery.service';
...
describe('SgtCoinDiscoveryService', () => {
  let service: SgtCoinDiscoveryService;
  ...
  beforeEach(() => {
    ...
    service = TestBed.inject(SgtCoinDiscoveryService);
  });
  ...
  it("surfaces 'sgt-not-deployed' when TAIL genesis coin id missing", async () => {
  ...
  it("surfaces 'governance-not-deployed' when launcher id missing", async () => {
    const res = await service.discover({
      voterInnerPuzzleHash: VOTER_INNER_PH,
      sgtGenesisCoinId: GENESIS_COIN_ID,
      trackerLauncherId: '',
    });
  ...
  it("surfaces 'no-coins' when coinset returns empty list", async () => {
    ...
    if (res.kind === 'no-coins') {
      expect(res.catSgtFreePuzzleHash).toMatch(/^0x[0-9a-f]{64}$/);
    }
  ...
  it('queries coinset with the derived CAT-sgt-free puzzle hash', async () => {
    coinset.getCoinRecordsByPuzzleHash.and.resolveTo([]);
  ...
  describe('catSgtFreePuzzleHashHex', () => {
    it('returns null when config is incomplete', () => {
      expect(
        service.catSgtFreePuzzleHashHex({
          voterInnerPuzzleHash: VOTER_INNER_PH,
          sgtGenesisCoinId: '',
          trackerLauncherId: TRACKER_LAUNCHER_ID,
        }),
    ...
    it('returns a deterministic 32-byte hex when configured', () => {
      const ph = service.catSgtFreePuzzleHashHex({
        voterInnerPuzzleHash: VOTER_INNER_PH,
        sgtGenesisCoinId: GENESIS_COIN_ID,
        trackerLauncherId: TRACKER_LAUNCHER_ID,
      });
