...
describe('CommitteeApiService', () => {
  ...
  it('POSTs spend_bundle to /admin/committee/propose without proposal_id', async () => {
    const bundle = fakeBundle();
    const pending = service.publishProposal(bundle);
  ...
  it('includes proposal_id in the propose request body when provided', async () => {
    const bundle = fakeBundle();
    const pending = service.publishProposal(bundle, 'draft-123');
    const req = http.expectOne(`${environment.faucetApi}/admin/committee/propose`);
    expect(req.request.body).toEqual({
      spend_bundle: bundle,
  ...
  it('includes proposal_metadata in the propose request body when provided', async () => {
    const bundle = fakeBundle();
    const metadata: PublishProposalMetadataJson = {
  ...
  it('omits proposal_metadata from the body when not provided', async () => {
    const bundle = fakeBundle();
    const pending = service.publishProposal(bundle, 'draft-123');
    const req = http.expectOne(`${environment.faucetApi}/admin/committee/propose`);
    expect('proposal_metadata' in (req.request.body as object)).toBe(false);
    req.flush({
      pushed: true,
      status: 'SUCCESS',
      spend_bundle_id: '0x' + 'ab'.repeat(32),
  ...
  it('surfaces propose mempool rejection (pushed=false) with status', async () => {
    const pending = service.publishProposal(fakeBundle());
    const req = http.expectOne(`${environment.faucetApi}/admin/committee/propose`);
    req.flush({
      pushed: false,
