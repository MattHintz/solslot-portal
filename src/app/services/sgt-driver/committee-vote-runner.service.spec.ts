...
import { SgtCoinDiscoveryService } from './sgt-coin-discovery.service';
...
function makeMockedRunner(overrides: {
  ...
  discovery: SgtCoinDiscoveryService;
  builder: SgtVoteSpendBuilderService;
    ...
    overrides.discoverKind === 'sgt-not-deployed'
      ...
      : overrides.discoverKind === 'governance-not-deployed'
        ...
        : overrides.discoverKind === 'no-coins'
          ? { kind: 'no-coins' as const, catSgtFreePuzzleHash: '0x' + 'ff'.repeat(32) }
          : {
              kind: 'found' as const,
              catSgtFreePuzzleHash: '0x' + 'ff'.repeat(32),
  ...
  const builder = {
    buildSgtLockCoinSpend:
      overrides.buildSgtLockImpl ??
      jasmine.createSpy('buildSgtLockCoinSpend').and.returnValue({
        coin: {
          parentCoinInfo: '0x' + '55'.repeat(32),
          puzzleHash: '0x' + 'ff'.repeat(32),
          amount: 1000,
  ...
  } as unknown as SgtVoteSpendBuilderService;
  ...
  // SgtDriverService's tree-hash helpers depend on real WASM treeHashAtom
  // exports.  The runner only uses three of them (trackerStructHash,
  // sgtLockedInnerHash, sgtTailHash) as orchestration glue — stub them
  // to return arbitrary 32-byte arrays so the spec stays focused on
  // orchestration.  Hash correctness is asserted by the SgtDriverService
  // spec instead.
  const sgt = {
    trackerStructHash: jasmine
      .createSpy('trackerStructHash')
      .and.returnValue(new Uint8Array(32).fill(0x10)),
    sgtLockedInnerHash: jasmine
      .createSpy('sgtLockedInnerHash')
      .and.returnValue(new Uint8Array(32).fill(0x20)),
    sgtTailHash: jasmine
      .createSpy('sgtTailHash')
...
describe('CommitteeVoteRunnerService', () => {
  ...
  beforeEach(() => {
    ...
    (
      environment.solslotProtocol as { sgtGenesisCoinId: string }
    ).sgtGenesisCoinId = '0x' + 'a0'.repeat(32);
  });

  afterEach(() => {
    (
      environment.solslotProtocol as { sgtGenesisCoinId: string }
    ).sgtGenesisCoinId = originalSgtTailGenesisCoinId;
  });
  ...
  it('passes the largest matching coin to the spend builder', async () => {
    const buildSpy = jasmine.createSpy('buildSgtLockCoinSpend').and.returnValue({
