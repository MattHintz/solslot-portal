...
import { PGT_FREE_INNER_PUZZLE_HEX } from './pgt-free-inner.puzzle-hex';
import { PGT_LOCKED_INNER_PUZZLE_HEX } from './pgt-locked-inner.puzzle-hex';
import { PGT_TAIL_PUZZLE_HEX } from './pgt-tail.puzzle-hex';
 ...
 * VOTE path needs to build a PGT lock spend whose announcement id
 ...
 *
 *   * The PGT TAIL curry hash (genesis-coin-id → tail puzzle hash).
 *   * The proposal-tracker singleton struct hash.
 *   * The curried pgt_free_inner inner puzzle hash (per voter).
 *   * The curried pgt_locked_inner inner puzzle hash (per voter + proposal).
 *   * The CAT-wrapped pgt_free coin puzzle hash (per voter).
 ...
 *
 * Spend builders (PGT lock spend + tracker VOTE coin spend) live in
 * a sibling service (``CommitteeVoteRunnerService``) — that's the
 * next phase.
 */
@Injectable({ providedIn: 'root' })
export class PgtDriverService {
  ...
  static readonly PGT_TAIL_MOD_HASH =
    '0x493afb89eed93ab86741b2aa61b8f5de495d33ff9b781dfc8919e602b2afa150';
  static readonly PGT_FREE_INNER_MOD_HASH =
    '0xdf22dc45fb795ce3392ac003ea78fb7d9fc1a13c366933341bcde43187df8e1e';
  ...
  /** Populis governance announcement prefix (single byte 0x50, 'P'). */
  static readonly PROTOCOL_PREFIX = new Uint8Array([0x50]);

  /** ``LOCK`` ASCII tag asserted by pgt_free_inner.clsp.  4-byte int. */
  ...
  /** Compute tree hash of the bundled pgt_tail bytecode (uncurried). */
  pgtTailModHash(): Uint8Array {
    return this.clvm().deserialize(hexToBytes(PGT_TAIL_PUZZLE_HEX)).treeHash();
  }
  /** Compute tree hash of the bundled pgt_free_inner bytecode. */
  pgtFreeInnerModHash(): Uint8Array {
    return this.clvm().deserialize(hexToBytes(PGT_FREE_INNER_PUZZLE_HEX)).treeHash();
  }
  /** Compute tree hash of the bundled pgt_locked_inner bytecode. */
  pgtLockedInnerModHash(): Uint8Array {
    return this.clvm().deserialize(hexToBytes(PGT_LOCKED_INNER_PUZZLE_HEX)).treeHash();
  }
  ...
  // ── PGT TAIL hash (per genesis coin) ─────────────────────────────────

  /**
   * Tree hash of ``pgt_tail.curry(genesis_coin_id)``.
   *
   * Mirrors ``pgt_driver.pgt_tail_hash``.
   */
  pgtTailHash(genesisCoinId: string): Uint8Array {
    return this.curryAndTreeHash(this.pgtTailQuotedMod(), [
   ...
   *
   * Mirrors ``pgt_driver.make_proposal_tracker_struct(...).get_tree_hash()``.
   */
  trackerStructHash(args: {
    ...
    const singletonMh = hexToBytes(
      args.singletonModHash ?? PgtDriverService.SINGLETON_MOD_HASH,
    );
    const launcherId = hexToBytes(args.trackerLauncherId);
    const launcherPh = hexToBytes(
      args.launcherPuzzleHash ?? PgtDriverService.SINGLETON_LAUNCHER_HASH,
    );
  ...
  // ── PGT_FREE / PGT_LOCKED inner hashes (curried) ─────────────────────

  /**
   * Tree hash of the pgt_free_inner curried for a specific voter.
   *
   *   pgt_free_inner.curry(
   *     PGT_FREE_INNER_MOD_HASH,
   *     PGT_LOCKED_INNER_MOD_HASH,
   *     TRACKER_STRUCT,
   *     VOTER_INNER_PUZZLE_HASH,
   *   )
   */
  pgtFreeInnerHash(args: {
    trackerStructHash: Uint8Array | string;
    voterInnerPuzzleHash: string;
    pgtLockedInnerModHash?: string;
  }): Uint8Array {
    const lockedMh = hexToBytes(
      args.pgtLockedInnerModHash ?? PgtDriverService.PGT_LOCKED_INNER_MOD_HASH,
    );
    const modHash = hexToBytes(PgtDriverService.PGT_FREE_INNER_MOD_HASH);
    const trackerStruct =
      typeof args.trackerStructHash === 'string'
        ? hexToBytes(args.trackerStructHash)
        : args.trackerStructHash;
    return this.curryAndTreeHash(this.pgtFreeInnerQuotedMod(), [
  ...
  /**
   * Tree hash of the pgt_locked_inner curried for a specific voter + proposal.
   *
   *   pgt_locked_inner.curry(
   *     PGT_LOCKED_INNER_MOD_HASH,
   *     PGT_FREE_INNER_MOD_HASH,
   *     TRACKER_STRUCT,
   *     VOTER_INNER_PUZZLE_HASH,
   ...
   *   )
   */
  pgtLockedInnerHash(args: {
    trackerStructHash: Uint8Array | string;
    voterInnerPuzzleHash: string;
    lockProposalHash: string;
    lockDeadlineSeconds: bigint | number;
    pgtFreeInnerModHash?: string;
  }): Uint8Array {
    const freeMh = hexToBytes(
      args.pgtFreeInnerModHash ?? PgtDriverService.PGT_FREE_INNER_MOD_HASH,
    );
    const modHash = hexToBytes(PgtDriverService.PGT_LOCKED_INNER_MOD_HASH);
    const trackerStruct =
      typeof args.trackerStructHash === 'string'
        ? hexToBytes(args.trackerStructHash)
        : args.trackerStructHash;
    return this.curryAndTreeHash(this.pgtLockedInnerQuotedMod(), [
      this.treeHashAtom(modHash),
  ...
  // ── CAT-wrapped pgt_free coin puzzle hash ────────────────────────────

  /**
   * Compute the on-chain puzzle hash of a voter's free PGT CAT coin:
   *
   *   CAT_MOD.curry(CAT_MOD_HASH, PGT_TAIL_HASH, PGT_FREE_INNER_HASH)
   *
   * Mirrors ``pgt_driver.cat_pgt_free_puzzle_hash``.  This is the
   * ``sender_ph`` half of the LOCK announcement id the tracker
   * asserts.
   */
  catPgtFreePuzzleHash(args: {
    pgtFreeInnerHash: Uint8Array | string;
    pgtTailHash: Uint8Array | string;
    catModHash?: string;
  }): Uint8Array {
    const catMh = hexToBytes(args.catModHash ?? PgtDriverService.CAT_MOD_HASH);
    const tail =
      typeof args.pgtTailHash === 'string'
        ? hexToBytes(args.pgtTailHash)
        : args.pgtTailHash;
    const inner =
      typeof args.pgtFreeInnerHash === 'string'
        ? hexToBytes(args.pgtFreeInnerHash)
        : args.pgtFreeInnerHash;
  ...
  /**
   * Compute the announcement id a voter's PGT lock spend emits.  This
   * value MUST match the announcement the governance tracker asserts
   * during PROPOSE / VOTE — that's how PGT weight binds to a vote.
   *
   * Formula (mirrors governance_singleton_inner.clsp::lock_announcement_id):
   *
   *   announcement_id = sha256(
   *     CAT_PGT_FREE_PUZHASH                                  ||
   *     PROTOCOL_PREFIX                                       ||
   *     sha256tree(LOCK_TAG proposal_hash amount deadline)
   *   )
   */
  async lockAnnouncementId(args: {
    catPgtFreePuzhash: Uint8Array | string;
    proposalHash: string;
    amountMojos: bigint | number;
    lockDeadlineSeconds: bigint | number;
  }): Promise<Uint8Array> {
    const sender =
      typeof args.catPgtFreePuzhash === 'string'
        ? hexToBytes(args.catPgtFreePuzhash)
        : args.catPgtFreePuzhash;
    const inner = this.lockArgsTreeHash(
      args.proposalHash,
      args.amountMojos,
      args.lockDeadlineSeconds,
    );
    const message = new Uint8Array(sender.length + PgtDriverService.PROTOCOL_PREFIX.length + inner.length);
    message.set(sender, 0);
    message.set(PgtDriverService.PROTOCOL_PREFIX, sender.length);
    message.set(inner, sender.length + PgtDriverService.PROTOCOL_PREFIX.length);
  ...
  lockArgsTreeHash(
    ...
    const list = clvm.list([
      clvm.int(BigInt(PgtDriverService.LOCK_TAG)),
  ...
  billFreeze(newPoolStatus: number): Uint8Array {
    return this.billTreeHash([
      this.treeHashAtom(new Uint8Array([PgtDriverService.BILL_FREEZE])),
  ...
  billVaultVersion(args: {
    ...
    return this.billTreeHash([
      this.treeHashAtom(new Uint8Array([PgtDriverService.BILL_VAULT_VERSION])),
   ...
   * ``pgt_driver.proposal_hash_from_bill`` semantics:
  ...
  private pgtTailQuotedMod(): Uint8Array {
    return this.quotedModFromHash(hexToBytes(PgtDriverService.PGT_TAIL_MOD_HASH));
  }
  private pgtFreeInnerQuotedMod(): Uint8Array {
    return this.quotedModFromHash(hexToBytes(PgtDriverService.PGT_FREE_INNER_MOD_HASH));
  }
  private pgtLockedInnerQuotedMod(): Uint8Array {
    return this.quotedModFromHash(hexToBytes(PgtDriverService.PGT_LOCKED_INNER_MOD_HASH));
  }
  ...
  private treeHashAtom(bytes: Uint8Array): Uint8Array {
    const sdk = this.wasm.sdk() as PgtSdkShape;
    if (!sdk.treeHashAtom) {
  ...
  private clvm(): ClvmShape {
    ...
    if (!sdk.Clvm) {
      throw new Error(
        'PgtDriverService: chia-wallet-sdk-wasm not loaded yet. ' +
