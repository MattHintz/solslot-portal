...
- Google Vault runtime configuration is generated by
  `scripts/prepare-google-vault-runtime.mjs`; it is disabled by default. When
  explicitly enabled for staging, `SOLSLOT_GOOGLE_OAUTH_CLIENT_ID` is a public
  Web OAuth client ID and `SOLSLOT_GOOGLE_VAULT_ENABLED=true`. Enable only
  `drive.appdata`, register the exact HTTPS browser origin, and never add a
  Google client secret to the portal. See [Google Vault staging](docs/google-vault-staging.md).
- `chiaAggSigMeAdditionalData` — Chia network replay-protection value used by
  the in-browser Google BLS signer. It must match the selected network's
  consensus constants (`37a90e...6615` for testnet11).
...
Google Vault is an opt-in, Testnet11-only browser-managed BLS path. A new
24-word mnemonic is created locally, derived through Chia's all-unhardened
`m/12381/8444/2/0` path, encrypted with exactly 600,000 PBKDF2-SHA-256 rounds
plus AES-256-GCM, and stored in Google Drive's private `appDataFolder`. The
password, plaintext mnemonic, and private key are never uploaded or persisted
by the portal. The decrypted signer exists only in page memory, so reloads
require Google authorization and the recovery password again. Ordinary sign-out
only clears page memory; revoking Google access is a separate explicit action.
Google-backed vaults use the same `chia_bls` registration, discovery hint, and
on-chain puzzle as Sage/Goby vaults. Browser signing has XSS risk and must only
be used with a dedicated testnet mnemonic after reviewing the signing prompt.
